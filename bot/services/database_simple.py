...
class DatabaseService:
    ...
    async def get_lesson_by_id(self, lesson_id: int) -> Optional[Any]:
        """Get lesson by ID"""
        async with AsyncSessionLocal() as session:
            try:
                result = await session.execute(
        ...
        async with AsyncSessionLocal() as session:
            try:
                query = text("""
                    ...
                    WHERE user_id = :user_id AND status IN ('pending', 'completed')
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                if target_type == "all":
                    ...
                    """)
                    result = await session.execute(query, {"target_id": target_id})
                    users = result.fetchall()
                else:
                    users = []
                ...
                for user in users:
                    insert_query = text("""
                        ...
                        (user_id, title, body, notification_type, target_type, target_id, sent_by, created_at)
                        VALUES (:user_id, :title, :body, :notification_type, :target_type, :target_id, :sent_by, NOW())
                    """)
                    await session.execute(insert_query, {
                        "user_id": user.id,
                        "title": title,
