...
class BotHandlers:
    ...
    async def register_handlers(self):
        ...
        # Scheduled tasks funksiyasini boshlash
    ...
    async def _handle_pending_action(self, message: Message, user: User):
        ...
        if action_type == "create_superuser":
            await self._handle_create_superuser_flow(message, user, state, text)
        elif action_type == "create_lesson":
        ...
        #     await self._handle_broadcast_flow(message, user, state, text)
        else:
    ...
    async def _handle_create_lesson_flow(self, message: Message, user: User, state: dict, text: str):
        """Dars yaratish jarayoni - bugungi sana avtomatik ishlatiladi"""
        if state["step"] == "title":
            state["title"] = text
            # Bugungi sana avtomatik oladi
            state["date"] = date.today()
            # Tavsif so'raymiz
            state["step"] = "description"
            await message.answer(
                f"📝 Dars tavsifini yuboring.\n"
                f"📅 Sana: *{state['date']}*\n"
                f"Agar tavsif kerak bo'lmasa `-` yuboring.",
                parse_mode="Markdown"
            )
            return
        ...
        self.pending_actions.pop(message.from_user.id, None)
        if lesson_id:
            await message.answer(f"✅ Yangi dars yaratildi.\n📅 Sana: {state['date']}")
        else:
    ...
    async def _handle_homework_flow(self, message: Message, user: User, state: dict, text: str):
        """Homework yaratish jarayoni - deadline validatsiyasi bilan"""
        if state["step"] == "title":
            state["title"] = text
        ...
        if state["step"] == "instructions":
            ...
            await message.answer(
                ...
                "⚠️ *Eski sanalar kiritilmasin!*",
                parse_mode="Markdown",
            )
            return

        # Due date validatsiyasi
        due_date = None
        if text != "-":
            try:
                due_date = datetime.strptime(text, "%d.%m.%Y %H:%M")
                # Eski sana tekshirish
                if due_date < datetime.now():
                    await message.answer(
                        f"❌ *Xatolik! Sana eskirgan!*\n\n"
                        f"Kiritilgan sana: *{text}*\n"
                        f"Hozirgi vaqt: *{datetime.now().strftime('%d.%m.%Y %H:%M')}*\n\n"
                        f"To'g'ri format: *25.04.2026 23:59*\n"
                        f"Yoki deadlinesiz bo'lsa `-` yuboring.",
                        parse_mode="Markdown"
                    )
                    # Qayta so'raymiz
                    return
                ...
                await message.answer(
                    "❌ *Deadline formati noto'g'ri!*\n\n"
                    "To'g'ri format: *25.04.2026 23:59*\n"
                    "Misol: *31.12.2026 23:59*\n"
                    "Yoki deadlinesiz bo'lsa `-` yuboring.",
                    parse_mode="Markdown"
                )
                return
        ...
        if state["type"] == "create_homework":
            ...
            if homework_id:
                due_date_str = due_date.strftime("%d.%m.%Y %H:%M") if due_date else "Deadline yo'q"
                await message.answer(f"✅ Homework yaratildi.\n📅 Deadline: {due_date_str}")
    ...
    async def _handle_broadcast_flow(self, message: Message, user: User, state: dict, text: str):
        """Birdaniga reklama yuborish jarayoni"""
        if state["step"] == "title":
            state["title"] = text
            state["step"] = "message"
            await message.answer(
        ...
        # Birdaniga yuborish
        # await self._broadcast_to_all_groups(user.id, message_text)
        self.pending_actions.pop(message.from_user.id, None)
        await message.answer("✅ Birdaniga reklama yuborildi!")
        await self.show_role_menu(message, user)
    ...
    async def show_role_menu(self, message: Message, user: User):
        ...
        if user.role in {"STUDENT", "TEACHER"}:
            ...
            if user.role in {"SUPER_ADMIN", "ADMIN"}:
                buttons.append([InlineKeyboardButton(text="📢 Birdaniga reklama", callback_data="send_broadcast")])
                buttons.append([InlineKeyboardButton(text="💰 Qarzdorlar", callback_data="show_debtors")])
                if user.role == "SUPER_ADMIN":
                    buttons.append([InlineKeyboardButton(text="👑 SuperAdmin qo'shish", callback_data="create_superuser")])
            buttons.append([InlineKeyboardButton(text="📊 Statistika", callback_data="stats")])

        await message.answer(
            f"{role_badge}Kerakli bo'limni tanlang:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
            parse_mode="Markdown",
        )

    async def handle_callback_query(self, callback: types.CallbackQuery):
        """Callback larni boshqarish - Super Admin uchun yangi xabar yuboradi"""
        try:
            ...
            # Super Admin uchun menyu tugmalari yangi xabar yuboradi
            is_super_admin = user.role == "SUPER_ADMIN"

            # Birdaniga reklama uchun - olib tashlandi
            # if data == "send_broadcast" and is_super_admin:
            #     await self.send_broadcast_message(message, user)
            #     return

            if data == "show_groups":
                if is_super_admin:
                    await self.show_groups_message(callback.message, user, page=1)
                else:
                    await self.show_groups(callback, user, page=1)
            elif data.startswith("groups_page:"):
                _, page = data.split(":")
                if is_super_admin:
                    await self.show_groups_message(callback.message, user, page=int(page))
                else:
                ...
                if is_super_admin:
                    await self.show_teachers_message(callback.message)
                else:
                    await self.show_teachers(callback)
            elif data == "show_debtors":
                await self.show_debtors(callback.message)
            elif data == "create_superuser":
                await self.start_create_superuser(callback)
            elif data == "stats":
                if is_super_admin:
                    await self.show_stats_message(callback.message)
                else:
                    await self.show_stats(callback, user)
            elif data == "back_to_menu":
                # Super Admin uchun menyuga qaytish
    ...
    async def show_groups(self, callback: types.CallbackQuery, user: User, page: int = 1):
        ...
        try:
            ...
            # Super Admin uchun maxsus xabar
            if user.role == "SUPER_ADMIN":
                header = f"👑 *Barcha guruhlar* ({page}/{total_pages})"
            elif user.role == "ADMIN":
                header = f"👤 *Barcha guruhlar* ({page}/{total_pages})"
            else:
                header = f"📚 Guruhlar ({page}/{total_pages})"

            await callback.message.edit_text(
                header,
                reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
                parse_mode="Markdown" if user.role in {"SUPER_ADMIN", "ADMIN"} else None,
            )
        except Exception as e:
            logger.error(f"Error in show_groups: {e}", exc_info=True)
            await callback.answer("Xatolik yuz berdi.", show_alert=True)
    ...
    async def start_create_lesson(self, callback: types.CallbackQuery, group_id: int):
        """Yangi dars yaratishni boshlash - bugungi sana avtomatik"""
	...
	# async def _broadcast_to_all_groups(self, user_id: int, message_text: str):
	#     """Birdaniga reklama yuborish - hamma guruhlarga"""
	#     try:
	#         telegram_ids = await db.get_all_students_for_broadcast()
	#         if not telegram_ids:
	#             logger.warning("Broadcast yuborish uchun telegram topilmadi.")
	#             return
	...
	#                 success_count += 1
	#             except Exception as e:
	#                 logger.warning(f"Broadcast xabari yuborilmadi ({telegram_id}): {e}")

	#         logger.info(f"✅ Broadcast yuborildi: {success_count}/{len(telegram_ids)} ta foydalanuvchiga")
	#     except Exception as e:
    ...
    async def show_groups_message(self, message: Message, user: User, page: int = 1):
        """Guruhlar ro'yxatini yangi xabar sifatida yuborish (Super Admin uchun)"""
        groups = await db.get_user_groups(user.id, user.role)
        if not groups:
    ...
    async def show_teachers_message(self, message: Message):
        """O'qituvchilar ro'yxatini yangi xabar sifatida yuborish (Super Admin uchun)"""
        teachers = await db.get_teachers()
        if not teachers:
            await message.answer("Hech qanday o'qituvchi yo'q.")
            return
        ...
        await message.answer(
            "\n".join(text).strip(),
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Ortga", callback_data="back_to_menu")]]),
            parse_mode="Markdown",
        )

    async def start_create_superuser(self, callback: types.CallbackQuery):
        """SuperAdmin yaratishni boshlash"""
        user = await self.get_user_by_telegram_id(callback.from_user.id)
        ...
        self.pending_actions[callback.from_user.id] = {
            "type": "create_superuser",
            "step": "full_name",
        }

        await callback.message.edit_text(
            "👑 *Yangi SuperAdmin qo'shish*\n\n"
            "1-qadam: Ism-Familiyani yuboring.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="⬅️ Ortga", callback_data="back_to_menu")]]
            ),
        )
        await callback.answer()

    async def show_debtors(self, message: Message):
        """Qarzdorlar ro'yxatini ko'rsatish (Super Admin uchun)"""
        try:
            debtors = await db.get_debtors()
            if not debtors:
                await message.answer(
                    "💰 *Qarzdorlar yo'q*\n\n"
                    "Barcha o'quvchilar to'lovlarini amalga oshirgan.",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Ortga", callback_data="back_to_menu")]]),
                    parse_mode="Markdown",
                )
                return

            # Qarzdorlar ro'yxatini tuzish
            text = [
                f"💰 *Qarzdorlar ro'yxati*",
            ...
            for idx, debtor in enumerate(debtors, start=1):
                ...
                text.append(f"{status_emoji} *{idx}. {debtor['student_name']}*")
                text.append(f"   📞 {debtor['student_phone']}")
                text.append(f"   📚 {debtor['group_name']}")
                text.append(f"   📅 {debtor['month']} {debtor['billing_year']}")
                text.append(f"   💳 To'lov: {amount_paid:,.0f} UZS / {amount_due:,.0f} UZS")
                text.append(f"   💰 Qarz: {debt:,.0f} UZS")

                if debtor['due_date']:
                    due_date = debtor['due_date'].strftime('%d.%m.%Y')
                    text.append(f"   ⏰ Deadline: {due_date}")
            ...
            text.append("━━━━━━━━━━━━━━━━━━━━")
            text.append(f"💰 *Jami qarz: {total_due:,.0f} UZS*")
            text.append(f"👥 {len(debtors)} ta qarzdor")
            text.append("━━━━━━━━━━━━━━━━━━━━")
            ...
            logger.info(f"✅ Qarzdorlar ro'yxati ko'rsatildi: {len(debtors)} ta")
        except Exception as e:
            logger.error(f"Error showing debtors: {e}", exc_info=True)
            await message.answer(
                "❌ Xatolik yuz berdi. Iltimos, keyinroq qayta urinib ko'ring.",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Ortga", callback_data="back_to_menu")]])
            )

    async def show_stats_message(self, message: Message):
        """Statistikani yangi xabar sifatida yuborish (Super Admin uchun)"""
        await message.answer(
            "📊 Umumiy statistika bo'limi keyingi bosqichda kengaytiriladi.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📢 Birdaniga reklama yuborish", callback_data="send_broadcast")],
                [InlineKeyboardButton(text="⬅️ Ortga", callback_data="back_to_menu")]]
            ),
        )

    async def send_broadcast_message(self, message: Message):
        """Birdaniga reklama yuborish"""
        self.pending_actions[message.from_user.id] = {
            "type": "broadcast",
            "step": "title",
        }
        await message.answer(
            "📢 Birdaniga reklama yuborish\n\n"
            "1-qadam: Reklama sarlavhasini yuboring.\n"
            "2-qadam: Matnini yuboring.\n"
    ...
    async def _handle_create_superuser_flow(self, message: Message, user: User, state: dict, text: str):
        """SuperAdmin yaratish jarayoni"""
        ...
        if state["step"] == "phone":
            ...
            state["phone"] = db.normalize_phone(text)
            state["step"] = "password"
            await message.answer(
                "🔐 Parolni yuboring.\n\n"
        ...
        # Password bosqich - yaratish
        if len(text) < 8:
            await message.answer(
                "❌ Parol juda qisqa!\n\n"
                "Kamida 8 ta belgi bo'lishi kerak.\n"
                "Qaytadan parolni yuboring."
            )
            return

        success = await db.create_superuser(
        ...
        if success:
            await message.answer(
                ...
                "Endi bu foydalanuvchi /start bilan botga kirishi mumkin.",
                reply_markup=types.ReplyKeyboardRemove(),
                parse_mode="Markdown",
            )
            await self.show_role_menu(message, user)
            ...
            await message.answer(
                ...
                "Ehtimol: telefon raqam allaqachon ro'yxatdan o'tgan.",
                reply_markup=types.ReplyKeyboardRemove(),
                parse_mode="Markdown",
            )
            await self.show_role_menu(message, user)
    ...
    async def run_scheduled_tasks(self):
        """Scheduled tasks - har kuni 9:00 va 18:00 da qarzdorlarni tekshirish"""
        if self.scheduled_task_running:
            return

        self.scheduled_task_running = True
        logger.info("🕐 Scheduled tasks boshlandi")
        ...
        try:
            while True:
                ...
                # Har 15 daqiqada vaqtni tekshirish
                if current_minute % 15 == 0:
                    # 9:00 ni tekshirish
                    if current_hour == 9:
                        logger.info(f"⏰ Qarzdorlar tekshirish: 9:00")
                        await self.check_and_notify_debtors()

                    # 18:00 ni tekshirish
                    if current_hour == 18:
                        logger.info(f"⏰ Qarzdorlar tekshirish: 18:00")
                        await self.check_and_notify_debtors()

                # Keyingi tekshiruvgacha kutish (taxminan 1 daqiqaga)
                await asyncio.sleep(60)  # 1 daqiqa

        except asyncio.CancelledError:
            logger.info("🛑 Scheduled tasks to'xtatildi")
        except Exception as e:
            logger.error(f"❌ Scheduled tasks xatolik: {e}", exc_info=True)
        finally:
            self.scheduled_task_running = False

    async def check_and_notify_debtors(self):
        """Qarzdorlarni tekshirib ularga habar yuborish"""
        try:
            debtors = await db.get_debtors_with_telegram_ids()

            if not debtors:
                logger.info("✅ Qarzdorlar yo'q")
                return

            logger.info(f"📊 {len(debtors)} ta qarzdor bor, ularga habar yuborilmoqda...")
            ...
            for debtor in debtors:
                ...
                # Habaar matnini tayyorlash
                status_emoji = "⚠️" if debtor['payment_status'] == 'OVERDUE' else "📋"

                message_text = (
                    f"📢 *Qarz to'lovi haqida*\n\n"
                    f"{status_emoji} {debtor['student_name']}\n"
                    f"📚 Guruh: {debtor['group_name']}\n"
                    f"📅 Oy: {debtor['month']} {debtor['billing_year']}\n"
                    f"💳 To'lov: {debtor['amount_paid']:,.0f} / {debtor['amount_due']:,.0f} UZS\n"
                    f"💰 Qarz: {debtor['debt']:,.0f} UZS\n\n"
                    f"⚡ *Iltimos, tezroq to'lang!*"
                )

                try:
                    await self.bot.send_message(chat_id=debtor['telegram_id'], text=message_text, parse_mode="Markdown")
                    success_count += 1
            ...
            logger.info(f"✅ Qarzdorlarga habar yuborildi: {success_count}/{len(debtors)} ta")
            ...
            admin_summary = (
                ...
                f"✅ {success_count} ta o'quvchiga habar yuborildi"
            )

            for admin_id in admin_ids:
                try:
                    await self.bot.send_message(chat_id=admin_id, text=admin_summary, parse_mode="Markdown")
                except Exception as e:
                    logger.warning(f"⚠️ Admin {admin_id} ga hisobot yuborilmadi: {e}")

        except Exception as e:
            logger.error(f"Error checking and notifying debtors: {e}", exc_info=True)
